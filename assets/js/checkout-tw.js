/* mydybox-taiwan-for-woocommerce: checkout-tw.js */
jQuery(document).ready(function ($) {
	...
	var $invoiceField = $('#billing_mydybox_invoice_type_field');
	...
	function toggleFields() {
		...
		if (type === 'company') {
			$('.mydybox-taiwan-for-woocommerce-company-field').show();
		} else {
			$('.mydybox-taiwan-for-woocommerce-company-field').hide();
		}

		if (['carrier_phone', 'carrier_cert', 'donate'].indexOf(type) !== -1) {
			$('.mydybox-taiwan-for-woocommerce-carrier-field').show();
		} else {
			$('.mydybox-taiwan-for-woocommerce-carrier-field').hide();
		}
	}

	$('body').on('change', '#billing_mydybox_invoice_type', toggleFields);
	toggleFields();
	...
	if (mydyboxCheckout.lookupEnabled !== 'yes') return;
	...
	function doTaxLookup($field) {
		...
		$wrap.append('<span class="ts-lookup-spinner" style="display:inline-block;margin-left:8px;color:#64748b;font-size:12px;">⏳ ' + mydyboxCheckout.lookingUp + '</span>');
		$title.prop('disabled', true);

		$.post(mydyboxCheckout.ajaxUrl, {
			action: 'mydybox_lookup_tax_id',
			tax_id: tax_id,
			nonce:  mydyboxCheckout.nonce,
			_t:     Date.now()
		}, function (response) {
			$wrap.find('.ts-lookup-spinner').remove();
			...
			if (response.success) {
				...
				$wrap.append('<span class="ts-lookup-msg" style="display:block;margin-top:4px;color:#16a34a;font-size:12px;">✓ ' + mydyboxCheckout.found + '</span>');
				setTimeout(function () { $wrap.find('.ts-lookup-msg').fadeOut(400, function () { $(this).remove(); }); }, 3000);
			} else {
				var errMsg = (typeof response.data === 'object' ? response.data.message : response.data) || mydyboxCheckout.notFound;
				$title.val('').attr('placeholder', mydyboxCheckout.notFound);
	...
	$('body').on('input', '#billing_mydybox_company_tax_id', function () {
	...
	$('body').on('change blur', '#billing_mydybox_company_tax_id', function () {
		clearTimeout(taxLookupTimer);
		doTaxLookup($(this));
	});
});
