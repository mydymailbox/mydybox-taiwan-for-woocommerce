...
class Rule_Engine {
	...
	private function load_core_components(): void {
		...
		$this->register_condition( new \Mydybox\Rule_Engine\Conditions\Address() );
		$this->register_condition( new \Mydybox\Rule_Engine\Conditions\Cart_Total() );
		$this->register_condition( new \Mydybox\Rule_Engine\Conditions\Max_Qty() );
		$this->register_condition( new \Mydybox\Rule_Engine\Conditions\Category() );
		$this->register_condition( new \Mydybox\Rule_Engine\Conditions\Product() );
		...
		$this->register_action( new \Mydybox\Rule_Engine\Actions\Hide_Payment() );
		$this->register_action( new \Mydybox\Rule_Engine\Actions\Hide_Shipping() );
		$this->register_action( new \Mydybox\Rule_Engine\Actions\Block_Checkout() );

		// Allow other modules to register components
		do_action( 'mydybox_register_rule_components', $this );
	}
	...
	public function get_rules( string $hook ): array {
		...
		$rules = get_option( "mydybox_rules_{$hook}" );
		if ( false === $rules ) {
			$rules = get_option( "mydybox_rules_{$hook}" );
		}
