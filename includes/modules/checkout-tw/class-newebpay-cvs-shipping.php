...
class NewebPay_CVS_Shipping {
	...
	public function boot(): void {
		if ( 'yes' !== get_option( 'ts_newebpay_cvs_enabled', 'no' ) ) {
			return;
		}
		...
		add_action( 'wp_ajax_ts_open_newebpay_cvs_map', [ $this, 'ajax_open_map' ] );
		add_action( 'wp_ajax_nopriv_ts_open_newebpay_cvs_map', [ $this, 'ajax_open_map' ] );

		add_action( 'wp_ajax_ts_newebpay_cvs_callback', [ $this, 'ajax_map_callback' ] );
		add_action( 'wp_ajax_nopriv_ts_newebpay_cvs_callback', [ $this, 'ajax_map_callback' ] );
	...
	public function register_shipping_method( array $methods ): array {
		$methods['taiwan_store_newebpay_cvs'] = '\Taiwan_Store_Core\Modules\Checkout_Tw\NewebPay_CVS_Shipping_Method';
		return $methods;
	}
	...
	public function ajax_open_map(): void {
		...
		$merchant_id = $is_test ? 'TestMerchant' : get_option( 'ts_newebpay_cvs_merchant_id', '' );
		$hash_key    = $is_test ? 'TestKey123456789' : get_option( 'ts_newebpay_cvs_hash_key', '' );
		$hash_iv     = $is_test ? 'TestIV1234567890' : get_option( 'ts_newebpay_cvs_hash_iv', '' );

		$callback_url = admin_url( 'admin-ajax.php?action=ts_newebpay_cvs_callback&nonce=' . wp_create_nonce( 'ts_newebpay_cvs_callback' ) );
		...
		$form .= '</form><script>document.getElementById("ts-newebpay-map-form").submit();</script>';
	...
	public function ajax_map_callback(): void {
		check_ajax_referer( 'ts_newebpay_cvs_callback', 'nonce' );
		...
		WC()->session->set( 'ts_newebpay_cvs_store', [
			'id'   => $store_id,
			'name' => $store_name,
			'addr' => $store_addr,
			'type' => $cvs_type,
	...
	public function save_store_to_order( \WC_Order $order, array $data ): void {
		...
		$order->update_meta_data( 'ts_cvs_store_provider', 'newebpay' );
	...
	public function display_store_in_order( \WC_Order $order ): void {
		if ( $order->get_meta( 'ts_cvs_store_provider' ) !== 'newebpay' ) return;
		if ( ! $order->get_meta( 'ts_cvs_store_id' ) ) return;
	...
	public function display_store_in_admin( \WC_Order $order ): void {
		...
		echo '<strong>🏪 ' . esc_html__( '超商取貨門市（藍新）', 'taiwan-store-core' ) . '</strong><br>';
	...
	private function render_store_block( \WC_Order $order ): void {
		$label = $this->get_cvs_label( $order->get_meta( 'ts_cvs_store_type' ) );
		...
		echo '<h3 style="margin:0 0 .5rem;font-size:1rem;">🏪 ' . esc_html__( '取貨門市資訊（藍新）', 'taiwan-store-core' ) . '</h3>';
