...
class Invoice_Export {
	...
	private const ACTION_GENERIC = 'mydybox_export_invoice_csv';
	...
	public function handle_global_export(): void {
		if ( sanitize_text_field( wp_unslash( $_GET['page'] ?? '' ) ) === 'mydybox-taiwan-for-woocommerce' && sanitize_text_field( wp_unslash( $_GET['action'] ?? '' ) ) === 'export-invoices' ) {
	...
	public function add_bulk_actions( array $actions ): array {
		$actions[ self::ACTION_GENERIC ] = __( 'Export Invoice: Generic (CSV)', 'mydybox-taiwan-for-woocommerce' );
		return $actions;
	}
	...
	private function output_csv( array $order_ids ): void {
		...
		fputcsv( $out, [
			__( 'Order Number', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Date', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Customer Name', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Email', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Invoice Type', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Carrier/Donation Code', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Tax ID', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Company Title', 'mydybox-taiwan-for-woocommerce' ),
			__( 'Total Amount', 'mydybox-taiwan-for-woocommerce' ),
		] );
		...
		foreach ( $order_ids as $order_id ) {
			...
			fputcsv( $out, [
				...
				(string) $order->get_meta( 'billing_mydybox_invoice_type' ),
				(string) $order->get_meta( 'billing_mydybox_carrier_number' ),
				(string) $order->get_meta( 'billing_mydybox_company_tax_id' ),
				(string) $order->get_meta( 'billing_mydybox_company_title' ),
				$order->get_total(),
			] );
		}
