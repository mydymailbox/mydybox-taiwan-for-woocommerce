...
class NewebPay_CVS_Shipping_Method extends \WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_title       = __( '超商取貨（藍新）', 'mydyma-taiwan-commerce-suite' );
		$this->method_description = __( '7-ELEVEN、全家、萊爾富、OK 超商取貨（藍新物流）', 'mydyma-taiwan-commerce-suite' );
	...
	public function init(): void {
		...
		$this->title      = $this->get_option( 'title', __( '超商取貨（藍新）', 'mydyma-taiwan-commerce-suite' ) );
	...
	public function init_form_fields(): void {
		$this->instance_form_fields = [
			'title' => [
				...
				'type'    => 'text',
				'default' => __( '超商取貨（藍新）', 'mydyma-taiwan-commerce-suite' ),
			],
			'cost' => [
	...
	public function is_available( $package ): bool {
		...
		if ( ! $is_test && ! get_option( 'mydyma_tcs_newebpay_cvs_merchant_id' ) ) {
			return false;
		}
