...
class CVS_Shipping {
	...
	public function boot(): void {
		if ( 'yes' !== get_option( 'mydybox_cvs_enabled', 'no' ) ) {
			return;
		}
		...
		add_action( 'wp_ajax_mydybox_open_cvs_map', [ $this, 'ajax_open_cvs_map' ] );
		add_action( 'wp_ajax_nopriv_mydybox_open_cvs_map', [ $this, 'ajax_open_cvs_map' ] );

		// AJAX: receive callback from ECPay map
		add_action( 'wp_ajax_mydybox_cvs_map_callback', [ $this, 'ajax_map_callback' ] );
		add_action( 'wp_ajax_nopriv_mydybox_cvs_map_callback', [ $this, 'ajax_map_callback' ] );
	...
	public function register_shipping_method( array $methods ): array {
		$methods['mydybox_cvs'] = '\Mydybox\Modules\Checkout_Tw\CVS_Shipping_Method';
		return $methods;
	}
	...
	public function enqueue_scripts(): void {
		...
		wp_enqueue_script(
			'mydybox-taiwan-for-woocommerce-cvs-map',
			MYDYBOX_URL . 'assets/js/cvs-map.js',
			[ 'jquery' ],
			filemtime( MYDYBOX_DIR . 'assets/js/cvs-map.js' ),
			true
		);

		wp_localize_script( 'mydybox-taiwan-for-woocommerce-cvs-map', 'mydyboxCvs', [
			'ajaxUrl'        => admin_url( 'admin-ajax.php' ),
			'nonce'          => wp_create_nonce( 'mydybox_cvs_map' ),
			'selectStore'    => __( '選擇門市', 'mydybox-taiwan-for-woocommerce' ),
			'changeStore'    => __( '更換門市', 'mydybox-taiwan-for-woocommerce' ),
			'noStoreSelected'=> __( '尚未選擇門市，請點擊「選擇門市」按鈕', 'mydybox-taiwan-for-woocommerce' ),
			'selectedStore'  => __( '已選擇門市', 'mydybox-taiwan-for-woocommerce' ),
			'callbackUrl'    => admin_url( 'admin-ajax.php?action=mydybox_cvs_map_callback' ),
		] );
	}
	...
	public function render_cvs_store_info(): void {
		...
		<div id="ts-cvs-store-wrap" style="display:none; margin-top: 1rem;">
			<div id="ts-cvs-store-info" class="ts-cvs-info-box">
				...
				<span id="ts-cvs-store-text"><?php esc_html_e( '尚未選擇門市，請點擊「選擇門市」按鈕', 'mydybox-taiwan-for-woocommerce' ); ?></span>
			</div>
			<button type="button" id="ts-cvs-select-btn" class="ts-cvs-btn">
				🗺️ <?php esc_html_e( '選擇門市', 'mydybox-taiwan-for-woocommerce' ); ?>
			</button>
	...
	public function ajax_open_cvs_map(): void {
		...
		$merchant_id = $is_test ? '3002607' : get_option( 'mydybox_cvs_merchant_id', '' );
		$hash_key    = $is_test ? 'pwFHCqoQZGmho4w6' : get_option( 'mydybox_cvs_hash_key', '' );
		$hash_iv     = $is_test ? 'EkRm7iFT261dpevs' : get_option( 'mydybox_cvs_hash_iv', '' );
		$endpoint    = $is_test ? self::API_STAGE : self::API_LIVE;

		$callback_url = admin_url( 'admin-ajax.php?action=mydybox_cvs_map_callback&nonce=' . wp_create_nonce( 'mydybox_cvs_callback' ) );
	...
	public function ajax_map_callback(): void {
		check_ajax_referer( 'mydybox_cvs_callback', 'nonce' );
		...
		WC()->session->set( 'mydybox_cvs_store', [
			'id'   => $store_id,
			'name' => $store_name,
			'addr' => $store_addr,
			'type' => $cvs_type,
	...
	public function display_store_in_order( \WC_Order $order ): void {
		$store_id = $order->get_meta( 'mydybox_cvs_store_id' );
		if ( ! $store_id ) return;
	...
	public function display_store_in_admin( \WC_Order $order ): void {
		...
		echo '<strong>🏪 ' . esc_html__( '超商取貨門市', 'mydybox-taiwan-for-woocommerce' ) . '</strong><br>';
	...
	private function render_store_block( \WC_Order $order ): void {
		$type  = $order->get_meta( 'mydybox_cvs_store_type' );
		...
		echo '<h3 style="margin:0 0 .5rem;font-size:1rem;">🏪 ' . esc_html__( '取貨門市資訊', 'mydybox-taiwan-for-woocommerce' ) . '</h3>';
