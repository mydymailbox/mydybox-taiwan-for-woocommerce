...
class Rules_Ajax {
	...
	public function boot(): void {
		add_action( 'wp_ajax_wc_tw_core_get_rules', [ $this, 'get_rules' ] );
		add_action( 'wp_ajax_wc_tw_core_save_rule', [ $this, 'save_rules' ] );
		add_action( 'wp_ajax_wc_tw_core_get_components', [ $this, 'get_components' ] );
		add_action( 'wp_ajax_wc_tw_core_import_samples', [ $this, 'import_samples' ] );
		add_action( 'wp_ajax_wc_tw_core_delete_rule', [ $this, 'delete_rule' ] );
	}
	...
	public function import_samples(): void {
		...
		if ( empty( $keys_by_cat ) || ! is_array( $keys_by_cat ) ) {
			wp_send_json_error( __( '無效的匯入請求', 'taiwan-store-core' ) );
		}

		$all_samples = include TAIWAN_STORE_CORE_DIR . 'includes/admin/data/sample-rules.php';
		$added_count = 0;

		foreach ( $keys_by_cat as $cat => $ids ) {
			if ( ! isset( $all_samples[ $cat ] ) ) continue;

			$existing_rules = get_option( "taiwan_store_core_rules_{$cat}", [] );
			...
			update_option( "taiwan_store_core_rules_{$cat}", $existing_rules );
		}

		// 返回目前頁面對應的規則列表
		$current_rules = get_option( "taiwan_store_core_rules_{$current_hook}", [] );
	...
	public function get_rules(): void {
		...
		$rules   = get_option( "taiwan_store_core_rules_{$section}", [] );
	...
	public function save_rules(): void {
		...
		update_option( "taiwan_store_core_rules_{$section}", $rules );
